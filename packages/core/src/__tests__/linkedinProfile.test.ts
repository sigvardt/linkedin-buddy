...
describe("LinkedInProfileService prepare helpers", () => {
  ...
  it("rejects vanity names shorter than 3 characters", () => {
    const db = new AssistantDatabase(":memory:");

    try {
      const service = new LinkedInProfileService(createTestRuntime(db));
      expect(() =>
        service.prepareUpdatePublicProfile({
          profileName: "default",
          vanityName: "ab",
        }),
      ).toThrow(/between 3 and 100 characters/);
    } finally {
      db.close();
    }
  });

  it("rejects vanity names longer than 100 characters", () => {
    const db = new AssistantDatabase(":memory:");

    try {
      const service = new LinkedInProfileService(createTestRuntime(db));
      const vanityName = `${"a-".repeat(50)}a`;
  ...
  it("accepts vanity names that are exactly 3 characters", () => {
    const db = new AssistantDatabase(":memory:");
    ...
    try {
      ...
      expect(prepared.preview).toMatchObject({
        vanity_name: "abc",
        public_profile_url: "https://www.linkedin.com/in/abc/",
      });
    } finally {
      db.close();
    }
  ...
  it("accepts vanity names that are exactly 100 characters", () => {
    const db = new AssistantDatabase(":memory:");

    try {
      const service = new LinkedInProfileService(createTestRuntime(db));
      const vanityName = "a-".repeat(50);
      const prepared = service.prepareUpdatePublicProfile({
        profileName: "default",
        vanityName,
      });
      ...
      expect(prepared.preview).toMatchObject({
        ...
        public_profile_url: `https://www.linkedin.com/in/${vanityName}/`,
      });
    } finally {
      db.close();
    }
  ...
  it("rejects vanity names that contain special characters", () => {
    const db = new AssistantDatabase(":memory:");
  ...
  it("rejects vanity names that contain spaces", () => {
    const db = new AssistantDatabase(":memory:");
  ...
  it("rejects vanity names that contain unicode characters", () => {
    const db = new AssistantDatabase(":memory:");
  ...
  it("accepts vanity names with hyphens and digits", () => {
    const db = new AssistantDatabase(":memory:");
    ...
    try {
      ...
      const prepared = service.prepareUpdatePublicProfile({
        profileName: "default",
        vanityName: "avery-cole-2024",
      });

      expect(prepared.preview).toMatchObject({
        vanity_name: "avery-cole-2024",
        public_profile_url: "https://www.linkedin.com/in/avery-cole-2024/",
      });
    } finally {
      db.close();
    }
  ...
  it("strips the /in/ prefix when vanityName is provided as /in/<slug>", () => {
    const db = new AssistantDatabase(":memory:");
    ...
    try {
      ...
      const prepared = service.prepareUpdatePublicProfile({
        profileName: "default",
        vanityName: "/in/avery-cole-example",
      });
  ...
  it("rejects URL-encoded vanity names that decode to invalid characters", () => {
    const db = new AssistantDatabase(":memory:");
    ...
    try {
      ...
      expect(() =>
        service.prepareUpdatePublicProfile({
          profileName: "default",
          publicProfileUrl: "https://www.linkedin.com/in/avery%5Fcole/",
        }),
      ).toThrow(/letters, digits, and hyphens/);
    } finally {
      db.close();
  ...
  it("rejects conflicting vanityName and publicProfileUrl inputs", () => {
    const db = new AssistantDatabase(":memory:");
    ...
    try {
      ...
      expect(() =>
        service.prepareUpdatePublicProfile({
          profileName: "default",
          vanityName: "avery-cole-example",
          publicProfileUrl: "https://www.linkedin.com/in/different-profile/",
        }),
      ).toThrow(/must all point to the same/);
    } finally {
      db.close();
    }
  });

  it("rejects conflicting vanityName, customProfileUrl, and publicProfileUrl inputs", () => {
    const db = new AssistantDatabase(":memory:");
    ...
    try {
      ...
      expect(() =>
        service.prepareUpdatePublicProfile({
          profileName: "default",
          vanityName: "avery-cole-example",
          customProfileUrl: "https://www.linkedin.com/in/second-profile/",
          publicProfileUrl: "https://www.linkedin.com/in/third-profile/",
        }),
      ).toThrow(/must all point to the same/);
    } finally {
      db.close();
  ...
  it("accepts matching vanityName and publicProfileUrl inputs", () => {
    const db = new AssistantDatabase(":memory:");
  ...
  it("rejects profile settings updates when industry is an empty string", () => {
    const db = new AssistantDatabase(":memory:");

    try {
      const service = new LinkedInProfileService(createTestRuntime(db));
      expect(() =>
        service.prepareUpdateSettings({
          profileName: "default",
          industry: "",
        }),
      ).toThrow(/at least one non-empty field/);
    } finally {
      db.close();
    }
  });

  it("trims whitespace when preparing profile settings industry updates", () => {
    const db = new AssistantDatabase(":memory:");
