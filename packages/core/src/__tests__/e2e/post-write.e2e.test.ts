...
describe("Post Write E2E (2PC post.create)", () => {
  ...
  describe("prepare-only (fresh rate window)", () => {
    beforeAll(() => {
      if (!e2e.canRun()) return;
      const runtime = e2e.runtime();
      // Reset the post-create rate limit counter consumed by the confirm test.
      // Setting windowStartMs=0 makes peek() treat the counter as expired (different window).
      runtime.db.upsertRateLimitCounter({
