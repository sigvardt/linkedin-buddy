...
export interface LocalDataDeletionPlanOptions {
  /**
   * Optional buddy home override used to resolve the owned local-state
   * footprint.
   */
  baseDir?: string;
  /**
...
export interface LocalDataDeletionPlan {
  /** Canonical buddy home used to scope in-base deletion targets. */
  baseDir: string;
...
function resolveRecoveryHint(code: string | undefined): string | undefined {
  switch (code) {
    ...
    case "EROFS":
      return "This path is on a read-only filesystem. Remount it writable or choose a writable buddy home.";
    default:
      return undefined;
  }
}
