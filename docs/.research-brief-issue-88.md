...
1. there is no dedicated `linkedin-buddy test:live --read-only` command yet
...
| --- | --- | --- |
| Stored session/cookie, no email+password login flow | The repo already has a persistent Playwright profile login flow via `login` / `openLogin()` in `packages/cli/src/bin/linkedin.ts:4101` and `packages/core/src/auth/session.ts:491`. The `linkedin-buddy` alias already exists in `packages/cli/package.json:7`. | Partial. The current implementation stores session state in a Playwright profile directory, not in a dedicated app-managed session blob. The CLI also still exposes headless email/password login in `packages/core/src/auth/session.ts:171` and `packages/cli/src/bin/linkedin.ts:4101`, so Tier 2 should explicitly avoid depending on that path. |
...
under the buddy home directory in `PROJECT.md:50`. `ProfileManager` then
...
- **met**: the operator can store and reuse a session without typing email and
  password every run
- **not met**: there is no explicit `linkedin-buddy auth:session` capture/import/export
...
That means issue #88’s proposed `linkedin-buddy test:live --read-only` naming is aligned
...
Issue #88 says “CLI command: `linkedin-buddy auth:session` to capture a session from a
manual login,” but the current product language and behavior are profile-based:

- `linkedin-buddy login --profile ...`
...
- `linkedin-buddy test live --read-only`
- hidden alias `linkedin-buddy test:live --read-only`
...
validation against a real LinkedIn session — the current `linkedin-buddy login --profile`
model is already sufficient to get started.

The dedicated `linkedin-buddy auth:session` UX can be added later if the project wants a
