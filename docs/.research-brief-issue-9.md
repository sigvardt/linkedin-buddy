...
- the main runtime home under `LINKEDIN_BUDDY_HOME` / `~/.linkedin-buddy/linkedin-buddy`
- a separate auth cooldown file at `~/.linkedin-buddy/rate-limit-state.json`
...
- base dir: `~/.linkedin-buddy/linkedin-buddy`
...
| `~/.linkedin-buddy/rate-limit-state.json` | `auth/rateLimitState.ts` | login cooldown state | Low to medium | Should be deleted even though it lives outside `baseDir` |
...
This matters because a command that only wipes `LINKEDIN_BUDDY_HOME` will leave behind local behavioral state that still changes later CLI behavior.
...
- `LINKEDIN_BUDDY_REDACTION_MODE` defaults to `off`
- `LINKEDIN_BUDDY_STORAGE_MODE` defaults to `full`
...
- user-facing validation errors use `LinkedInBuddyError`
...
- path-sensitive because one file lives outside `LINKEDIN_BUDDY_HOME`
