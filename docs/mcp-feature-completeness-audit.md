...
- `packages/mcp/src/bin/linkedin-mcp.ts`
- `packages/mcp/src/toolArgs.ts` — arg-reading utilities (`readString`, `readRequiredString`, `readPositiveNumber`, etc.)
- `packages/mcp/src/toolSchema.ts` — schema types and validation (`LinkedInMcpInputSchema`, `validateToolArgValueAgainstSchema`)
- `packages/mcp/src/toolResults.ts` — result formatting, error recovery hints (`toToolResult`, `toErrorResult`, `getRecoveryHint`)
- `packages/mcp/src/toolRuntime.ts` — runtime creation, timeout enforcement (`createRuntime`, `withRuntime`, `withCdpSchemaProperties`)
