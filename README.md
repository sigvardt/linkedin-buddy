...
- `packages/mcp` (`@linkedin-buddy/mcp`) with `linkedin-mcp` bin
...
```bash
npm exec -w @linkedin-buddy/cli -- linkedin seed activity --profile default --spec docs/profile-seeds/issue-212-signikant-test-activity.json --delay-ms 4500 --yes --output reports/activity-seed.json
```
